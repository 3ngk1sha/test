"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.InvitationRepository = void 0;
const mongoose_1 = require("@nestjs/mongoose");
const mongoose_2 = require("mongoose");
const base_repository_1 = require("../../utils/generics/base-repository");
const invitation_schema_1 = require("../schemas/invitation.schema");
const hash_1 = require("../utilities/hash");
let InvitationRepository = class InvitationRepository extends base_repository_1.BaseRepository {
    constructor(model) {
        super(model, invitation_schema_1.Invitation, invitation_schema_1.INVITATION_POPULATE, invitation_schema_1.InvitationFull);
        this.model = model;
    }
    async preCreate(_doc) {
        if (_doc?.token) {
            _doc.token = (0, hash_1.hash)(_doc.token);
        }
        else {
            throw new Error('No token provided');
        }
    }
    findQuery(filter) {
        const filterWithHashedToken = filter.token
            ? { ...filter, token: (0, hash_1.hash)(filter.token.toString()) }
            : filter;
        return super.findQuery(filterWithHashedToken);
    }
};
exports.InvitationRepository = InvitationRepository;
exports.InvitationRepository = InvitationRepository = __decorate([
    __param(0, (0, mongoose_1.InjectModel)(invitation_schema_1.Invitation.name)),
    __metadata("design:paramtypes", [mongoose_2.Model])
], InvitationRepository);
//# sourceMappingURL=invitation.repository.js.map